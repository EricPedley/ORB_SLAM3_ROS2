...
public:
  Localize() : Node("localize")
  {
    ...
    // create publishers
    luci_pub_ = create_publisher<sensor_msgs::msg::PointCloud2>(
      "luci_point_cloud", 10);
    orb_pub_ = create_publisher<sensor_msgs::msg::PointCloud2>(
      "orb_point_cloud", 10);
    ...
    timer_ = create_wall_timer(
        100ms, std::bind(&Localize::timer_callback, this));
...
private:
  void timer_callback()
  {
    ...
    Eigen::Quaternionf q(*point_cloud_transform_);
  ...
  void match_point_cloud()
  {
    ...
    point_cloud_transform_ = std::make_shared<PM::TransformationParameters>(icp(*data_dp_, *reference_dp_));
