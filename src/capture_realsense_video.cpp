...
public:
  CaptureVideo() : Node("capture_video"), count(0) {
    // declare parameters
    ...
    image_sub = create_subscription<sensor_msgs::msg::Image>(
        ...
        std::bind(&CaptureVideo::image_callback, this, _1));
    ...
    std::string file_name =
        std::string(PROJECT_PATH) + "/videos/" + time_string + "_" + video_name;
    output_video.open(file_name, cv::VideoWriter::fourcc('m', 'p', '4', 'v'),
                      30, cv::Size(1280, 720), false);
    ...
    std::string image_timestamp_file_name =
        std::string(PROJECT_PATH) + "/videos/" + time_string + "_" +
    ...
    if (!image_timestamp_file.is_open()) {
      RCLCPP_ERROR(get_logger(),
                   "Could not open image timestamp file for writing");
      rclcpp::shutdown();
    }
    ...
    std::string imu_file_name =
        std::string(PROJECT_PATH) + "/videos/" + time_string + "_" +
        video_name.substr(0, video_name.length() - 4) + ".csv";
...
private:
  void imu_callback(const sensor_msgs::msg::Imu msg) {
    ...
    pitch =
        std::atan2(-linear_acceleration.x,
                   std::sqrt(linear_acceleration.y * linear_acceleration.y +
    ...
    timestamp = msg.header.stamp.sec + msg.header.stamp.nanosec * 1e-9;
    imu_file << std::fixed << std::setprecision(9) << msg.linear_acceleration.x
  ...
  void image_callback(const sensor_msgs::msg::Image::SharedPtr msg) {
    ...
    image_timestamp_file << std::fixed << std::setprecision(9) << timestamp
                         << std::endl;
